<?php
    ...
    if(isset($_POST['prova_id'])){
        ...
        if ($stmt->rowCount() > 0)
        {
            ...
            foreach ($rows as $row) 
            {
                $teams[$team_index] = $row['team_name'];
                $team_index++;
            }
        } else {
        ...
        for ($row = 2; $row <= $highestRow; $row++) { 
            ...
            //echo $team_id." ".$rowData[0][7]." ".$rowData[0][4]." <br> ";
            $query = "INSERT INTO athletes (athlete_chip, athlete_license, athlete_bib, athlete_name, athlete_sex, athlete_category, athlete_team_id, athlete_race_id, athlete_t0) VALUES (:chip, :license, :bib, :name, :sex, :category, :team_id, :race, :t0)";
            $t0 = date('H:i:s', PHPExcel_Shared_Date::ExcelToPHP($rowData[0][9]));
