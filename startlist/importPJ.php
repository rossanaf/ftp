<?php
    ...
    if(isset($_POST['prova_id'])){
        ...
        if ($row) 
        {
            $race_id = $row['race_id'] + 1;
        }

        $stmt = $db->prepare("INSERT INTO races(race_id, race_name, race_type, race_gender) VALUES (:id, 'P.Jovem', :type, 'X')");
        $stmt->execute(
            array(
                ':id' => $race_id, 
                ':type' => $_POST['prova_id']
        ...
        $stmt = $db->prepare("INSERT INTO youthraces (youthrace_race_id, youthrace_name) VALUES (?, 'Prova Jovem')");
        $stmt->execute([$race_id]);
