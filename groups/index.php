...
<div class="container">
    <div class="col-md-12">
    	<div align="right">
            <button type="button" id="add_button" data-toggle="modal" data-target="#userModal" class="btn btn-info btn-lg">Adicionar novo Escalão</button>
        </div>
        <br/>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
                <div class="form-group row">
                    <label for="name" class="col-sm-2 col-form-label">Escalão:</label>
                    <div class="col-sm-10">
	                    <input type="text" name="name" id="name" class="form-control" placeholder="Escalão"/>
                    </div>
                </div>
			</div>
...
<script type="text/javascript" language="javascript" >
	...
	$(document).ready(function(){
		$('#add_button').click(function(){
			$('#user_form')[0].reset();
			$('.modal-title').text("Adicionar novo Escalão");
		...
		$(document).on('click', '.update', function()
		{
		    ...
		    $.ajax({
		        ...
		        success:function(data){
		            ...
		            $('#name').val(data.name);
		            $('.modal-title').text("Editar nome do Escalão");
		            $('#user_id').val(user_id);
		...
		$(document).on('click', '.delete', function(){
		    var user_id = $(this).attr("id");
		    if(confirm("Tem a certeza que quer eliminar o Escalão selecionado?")){
		        $.ajax({
		            url:"delete.php",
		            method:"POST",
		            data:{user_id:user_id},
