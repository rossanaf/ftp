  ...
  include ($_SERVER['DOCUMENT_ROOT']."/functions/getTotalRecords.php");	
	...
	foreach($result as $row){          
		...
		if (($row['race_type'] == "jovem") || ($row['race_type'] == "jovemaq")){
			$sub_array[] = '<center><button title="Guns PJ" type="button" name="gunsYouth" id="'.$row["race_id"].'" class="gunsYouth"><img src="../images/horn.jpg"></button></center>';
      ...
      $sub_array[] = '<center><button title="Guns" type="button" name="guns" id="'.$row["race_id"].'" class="guns"><img src="../images/horn.jpg"></button></center>';
      $sub_array[] = '<button title="Editar Prova" type="button" name="update" id="'.$row["race_id"].'" class="btn btn-info btn-xs update"><i class="fa fa-pencil"></i></button>';
    }
