<?php 
  function processLiveTimes($chip, $t1, $t2, $t3, $t4, $t5, $db) {
    ...
    if($liveAthlete['live_t2'] == 'time') {
      if ($t1 !== '-' && $t2 !== '-') {
        $segmentTime = date('H:i:s', strtotime($t2)-strtotime($t1));
    ...
    if($liveAthlete['live_t3'] === 'time') {
      if ($t2 !== '-' && $t3 !== '-') {
        $segmentTime = date('H:i:s', strtotime($t3)-strtotime($t2));
    ...
    if($liveAthlete['live_t4'] === 'time') {
      if ($t3 !== '-' && $t4 !== '-') {
        $segmentTime = date('H:i:s', strtotime($t4)-strtotime($t3));
    ...
    if($liveAthlete['live_t5'] === 'time') {
      if ($t4 !== '-' && $t5 !== '-') {
        $segmentTime = date('H:i:s', strtotime($t5)-strtotime($t4));
  ...
  function processTriathlonTimes($gun, $gender, $raceId, $raceType, $raceRelay, $live, $db) {
    ...
    foreach ($athletes as $athlete) {
      ...
      // if($athlete['athlete_t0'] !== '-') {
        if (($athlete['athlete_finishtime'] !== 'LAP') && ($athlete['athlete_finishtime'] !== 'DNS') && ($athlete['athlete_finishtime'] !== 'DSQ') && ($athlete['athlete_finishtime'] !== 'DNF')) {
          ...
          if ($raceType === 'crind' || $raceRelay === 'X') {
            ...
            $total = date('H:i:s', strtotime($chipTime)-strtotime($gun));
          }
      ...
      //                       $started = 0;
      //                   }
      //                   // $chipTime = date('H:i:s', strtotime($chipTime)-strtotime($gun)); 
      ...
      //           } */
          ...
          if($athlete['athlete_started'] == 0) {
            ...
            // if ($raceType == 'crind') {
            //   $chipTime = date('H:i:s', strtotime($chipTime)-strtotime($athlete['athlete_t0']));
            // } 
          }
          // **** Tempo T2 ****//
      ...
      //     //     foreach ($athletes as $athlete) 
      //     //     {
      //     //         $ttotal = date('H:i:s', strtotime($athlete['athlete_finishtime'])-strtotime($gun));
  ...
  function processLiveTimesMxRelay($raceId, $db) {
    // MELHORAR CODIGO PARA NAO VIR SEMPRE AQUI
    // SO PODE CORRER SE TABELA TIME NAO TIVER TEMPOS
    $stmt = $db->prepare('SELECT athlete_t1, athlete_t2, athlete_t3, athlete_t4, athlete_t5, athlete_chip, athlete_finishtime FROM athletes WHERE athlete_started>0 AND athlete_race_id=?');
    $stmt->execute([$raceId]);
    $athletes = $stmt->fetchAll();
    foreach ($athletes as $athlete) {
      if (($athlete['athlete_finishtime'] !== 'LAP') && ($athlete['athlete_finishtime'] !== 'DNS') && ($athlete['athlete_finishtime'] !== 'DSQ') && ($athlete['athlete_finishtime'] !== 'DNF')) {
        ...
        $stmtLive = $db->prepare('SELECT live_t2, live_t3, live_t4, live_t5 FROM live WHERE live_chip=? AND live_race=? LIMIT 1');
        $stmtLive->execute([$chip, $raceId]);
        $liveAthlete = $stmtLive->fetch();
        if($liveAthlete['live_t2'] == 'time') {
          if ($t1 !== '-' && $t2 !== '-') {
            $segmentTime = date('H:i:s', strtotime($t2)-strtotime($t1));
  ...
  function processTriathlonTimesMxRelay($raceId, $raceType, $live, $gun, $db) {
    ...
    foreach ($athletes as $athlete) {
      if (($athlete['athlete_finishtime'] !== 'LAP') && ($athlete['athlete_finishtime'] !== 'DNS') && ($athlete['athlete_finishtime'] !== 'DSQ') && ($athlete['athlete_finishtime'] !== 'DNF')) {
        ...
        if ($t0['athlete_t0'] === '-') {
          ...
          $total = date('H:i:s', strtotime($chipTime)-strtotime($t0['athlete_t0']));
        }
        $location = $athlete['Location']; 
        ...
        if($location === "TimeT5" && $t5 === "-") {
          ...
          if($raceType === 'iturelay') {
            $stmtUpdatePreviousT0 = $db->prepare("UPDATE athletes SET athlete_t0=? WHERE athlete_bib=? AND athlete_arrive_order=? AND athlete_race_id=?");
            $stmtUpdatePreviousT0->execute([$t5, $bib, $nextOrder, $raceId]);
          }
        }
      }
      $thisAthlete = $athlete['Chip']; 
    ...
    if ($live == 1) {
      processLiveTimesMxRelay($raceId, $db);
      // CODIGO REPETIDO
      ...
      foreach ($finishers as $finisher) {
        ...
        $teamTotalTime = date('H:i:s', strtotime($finisher['athlete_t5']) - strtotime($gun));   
        $stmtUpdate = $db->prepare('UPDATE live SET live_t0=? WHERE live_bib=? AND live_race=?');
        $stmtUpdate->execute([$teamTotalTime, $bib, $raceId]);
      }
      // FIM CODIGO REPETIDO
      if ($livePositions == 1) {
  ...
  function processTriathlonTimesRelayPt($raceId, $raceType, $live, $gun, $gender, $db) {
    ...
    if ($live == 1) {
      ...
      foreach ($finishers as $finisher) {
        $bib = $finisher['live_bib'];
        $teamTotalTime = date('H:i:s', strtotime($finisher['athlete_t5']) - strtotime($race['race_gun_m']));   
