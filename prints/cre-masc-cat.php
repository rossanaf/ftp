<?php
  ...
  class PDF extends FPDF {
    ...
    function Header() {
      ...
      pdfHeader_V($this, $_GET['race_id'], 'M', 'Classificações Escalões Masculinos');
    }
    // Page footer
    function Footer() {
...
$querygun = $db->prepare("SELECT race_type FROM races WHERE race_id = ? LIMIT 1");
  ...
  // LE APENAS AS EQUIPAS QUE OS 3 PRIMEIROS CHEGARAM A META, POR ESCALOES, SEM ABSOLUTOS
  $athlete_category = array("CAD", "JUN", "ELITE","20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54", "55-59", "60-64", "65-69", "70-74", "75-79", "80-84", "85-89", "90-94", "95-99", "S23", "SEN", "V1", "V2", "V3", "V4", "V5", "VET","PTVI");
  $athlete_category_extenso = array("Cadetes", "Juniores", "ELITE", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54", "55-59", "60-64", "65-69", "70-74", "75-79", "80-84", "85-89", "90-94", "95-99", "Sub-23", "Seniores", "Veteranos 1", "Veteranos 2", "Veteranos 3", "Veteranos 4", "Veteranos 5", "Veteranos", "PTVI");
  for($j=0;$j<count($athlete_category);$j++) {
    $query = $db->prepare('SELECT athlete_team_id, athlete_xtras FROM athletes WHERE athlete_started>=5 AND athlete_race_id=? AND athlete_sex=? AND athlete_totaltime<>? AND athlete_race_id=? AND athlete_category=? ORDER BY athlete_totaltime ASC');
    $query->execute([$race_id, 'M', '-', $race_id, $athlete_category[$j]]);
    ...
    foreach ($teams as $team) {
      if (!in_array($team['athlete_team_id'], $teamsArrived)) {
        ...
        if ($pos === 1) {
          ...
          $pdf->Cell(20,10,$athlete_category_extenso[$j],0,1,'C');
          $pdf->SetFont('Times','',8);
        }
    ...
    $query = $db->prepare('SELECT DISTINCT athlete_team_id, athlete_xtras FROM athletes WHERE athlete_race_id=? AND athlete_sex=? AND athlete_arrive_order<? AND athlete_category=? ORDER BY athlete_started DESC, athlete_finishtime ASC');
    $query->execute([$race_id, 'M', 3, $athlete_category[$j]]);
    $teams = $query->fetchAll();
    foreach ($teams as $team) {
      if (!in_array($team['athlete_team_id'], $teamsArrived)) {
