<?php
  ...
  class PDF extends FPDF {
    ...
    function Header() {
      ...
      pdfHeader_V($this, $_GET['race_id'], 'F', 'Classificações Geral Femininos');
    }
    // Page footer
    function Footer() {
  ...
  $query = $db->prepare("SELECT * FROM athletes WHERE athlete_started >= 5 AND athlete_race_id=? AND athlete_sex=? ORDER BY athlete_t5 ASC");
  ...
  $query = $db->prepare('SELECT athlete_team_id, athlete_xtras FROM athletes WHERE athlete_started>=5 AND athlete_race_id=? AND athlete_sex=? AND athlete_totaltime<>? ORDER BY athlete_totaltime ASC');
  $query->execute([$race_id, 'F', '-']);
  $teams = $query->fetchAll();
  $pos = 1;
  foreach ($teams as $team) {
    $posTeam = 1;
    $query = $db->prepare('SELECT * FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athlete_team_id=? AND athlete_xtras=? AND athlete_sex=? AND athlete_started=5 ORDER BY athlete_finishtime ASC');
