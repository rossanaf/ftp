...
function Header()
{
    ...
    $this->SetX(190);
    $this->Cell(10,5,gmdate('H:i:s', strtotime($rowrace['race_gun'])-strtotime('01:00:00')),0,0,'R',true);
    ...
    $this->Cell(190,8,utf8_decode("Classificações Escalões"),0,0,'C');
    $this->Ln(10);
    $this->SetDrawColor(0);
...
$athlete_category = array("JUV", "CAD", "JUN", "SEN", "VET");
$athlete_category_extenso = array("Juvenis", "Cadetes", "Juniores", "Seniores", "Veteranos");
...
{
    $query = $db->prepare("SELECT athlete_team_id, athlete_bib FROM athletes WHERE athlete_started >= 4 AND athletes.athlete_race_id = ? AND athlete_rly_gender = ? ORDER BY athlete_finishtime ASC");
    ...
    foreach ($rows as $row) 
    {
        ...
        if (!in_array($row['athlete_team_id'], $teams)) 
        {
            ...
            foreach ($team as $relay) 
            {
                ...
                {
                    $pdf->Cell(19,5,$relay['athlete_totaltime'],1,1,'C',$fill);
                }
            }
        }
    }
    ...
    for($j=0;$j<count($penalty);$j++)
    {
        ...
        $query_pen = $db->prepare("SELECT athlete_team_id, athlete_bib FROM athletes WHERE athlete_finishtime = ? AND athlete_race_id = ? AND athlete_bib LIKE '%C' AND athlete_rly_gender = ? ORDER BY athlete_finishtime ASC");
