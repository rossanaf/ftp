...
function Header()
{
    ...
    $querygun = $db->prepare("SELECT gunshot_benf FROM gunshots WHERE gunshot_race_id = ? LIMIT 1");
    ...
    $this->SetX(190);
    $this->Cell(10,5,$rowgun['gunshot_benf'],0,0,'R',true);
    ...
    $this->Cell(190,8,utf8_decode("Classificações Benjamins Femininos"),0,0,'C');
    $this->Ln(10);
    $this->SetDrawColor(0);
...
$query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_started >= 5 AND athletes.athlete_race_id = ? AND athletes.athlete_sex = 'F' AND athletes.athlete_category = 'BEN'");
...
foreach ($rows as $row) 
{
    ...
    // {
        $athlete_totaltime = gmdate('H:i:s', strtotime($row['athlete_finishtime'])-strtotime($rowrace['gunshot_benf']));
...
$query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_started >= 5 AND athletes.athlete_race_id = ? AND athletes.athlete_sex = 'F' AND athletes.athlete_category = 'BEN' ORDER BY athlete_finishtime ASC");
$query->execute([$race_id]);
$rows = $query->fetchAll();
foreach ($rows as $row) 
{
...
{
    $query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_race_id = ? AND athletes.athlete_finishtime = ? AND athletes.athlete_sex = 'F' AND athletes.athlete_category = 'BEN' ORDER BY athletes.athlete_started DESC");
