...
function Header()
{
    ...
    $querygun = $db->prepare("SELECT gunshot_inim FROM gunshots WHERE gunshot_race_id = ? LIMIT 1");
    ...
    $this->SetX(190);
    $this->Cell(10,5,$rowgun['gunshot_inim'],0,0,'R',true);
    
    $segment1 = ucwords($rowrace['youthrace_s1_ini'])." - ".$rowrace['youthrace_d1_ini'];
    if ($rowrace['youthrace_s2_ini'] !== 'n.a.')
    {
        $segment2 = ucwords($rowrace['youthrace_s2_ini'])." - ".$rowrace['youthrace_d2_ini'];
    }
    $segment3 = ucwords($rowrace['youthrace_s3_ini'])." - ".$rowrace['youthrace_d3_ini'];
    ...
    $this->Cell(190,8,utf8_decode("Classificações Iniciados Masculinos"),0,0,'C');
    $this->Ln(10);
    $this->SetDrawColor(0);
...
$query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_started >= '5' AND athletes.athlete_race_id = ? AND athletes.athlete_sex = 'M' AND athletes.athlete_category = 'INI'");
...
foreach ($rows as $row) 
{
    ...
    // {
        $athlete_totaltime = gmdate('H:i:s', strtotime($row['athlete_finishtime'])-strtotime($rowrace['gunshot_inim']));
...
$query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_started >= '5' AND athletes.athlete_race_id = ? AND athletes.athlete_sex = 'M' AND athletes.athlete_category = 'INI' ORDER BY athlete_finishtime ASC");
$query->execute([$race_id]);
$rows = $query->fetchAll();
foreach ($rows as $row) 
{
...
{
    $query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_race_id = ? AND athletes.athlete_finishtime = ? AND athletes.athlete_sex = 'M' AND athletes.athlete_category = 'INI' ORDER BY athletes.athlete_started DESC");
