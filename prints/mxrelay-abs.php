...
function Header()
{
	...
	$header = array('#','Lic.','Dors.','Nome','Esc.','Equipa','T.Individual','T.Equipa', 'Diff.');
...
$query = $db->prepare("SELECT athlete_team_id, athlete_bib FROM athletes WHERE athlete_started >= 4 AND athletes.athlete_race_id = ? ORDER BY athlete_finishtime ASC");
...
{
    $query = $db->prepare("SELECT athlete_team_id, athlete_bib FROM athletes WHERE athlete_finishtime = ? AND athletes.athlete_race_id = ? AND athlete_bib LIKE '%C' ORDER BY athlete_finishtime ASC");
