...
{
    $query = $db->prepare("SELECT athlete_team_id, athlete_bib FROM athletes WHERE athlete_started >= 4 AND athletes.athlete_race_id = ? AND athletes.athlete_sex = 'F' AND athlete_category = ? ORDER BY athlete_finishtime ASC");
    ...
    for($j=0;$j<count($penalty);$j++)
    {
        ...
        $query_pen = $db->prepare("SELECT athlete_team_id, athlete_bib FROM athletes WHERE athlete_finishtime = ? AND athlete_race_id = ? AND athlete_sex = 'F' AND athlete_bib LIKE '%C' AND athlete_category = ? ORDER BY athlete_finishtime ASC");
