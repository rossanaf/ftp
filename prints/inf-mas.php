...
function Header()
{
    ...
    $querygun = $db->prepare("SELECT gunshot_infm FROM gunshots WHERE gunshot_race_id = ? LIMIT 1");
    ...
    $this->SetX(190);
    $this->Cell(10,5,$rowgun['gunshot_infm'],0,0,'R',true);
    
    $segment1 = ucwords($rowrace['youthrace_s1_inf'])." - ".$rowrace['youthrace_d1_inf'];
    if ($rowrace['youthrace_s2_inf'] !== 'n.a.')
    {
        $segment2 = ucwords($rowrace['youthrace_s2_inf'])." - ".$rowrace['youthrace_d2_inf'];
    }
    $segment3 = ucwords($rowrace['youthrace_s3_inf'])." - ".$rowrace['youthrace_d3_inf'];
    ...
    $this->Cell(190,8,utf8_decode("Classificações Infantis Masculinos"),0,0,'C');
    $this->Ln(10);
    $this->SetDrawColor(0);
...
$query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_started >= '5' AND athletes.athlete_race_id = ? AND athletes.athlete_sex = 'M' AND athletes.athlete_category = 'INF'");
...
foreach ($rows as $row) 
{
    ...
    // {
        $athlete_totaltime = gmdate('H:i:s', strtotime($row['athlete_finishtime'])-strtotime($rowrace['gunshot_infm']));
...
$query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_started >= '5' AND athletes.athlete_race_id = ? AND athletes.athlete_sex = 'M' AND athletes.athlete_category = 'INF' ORDER BY athlete_finishtime ASC");
$query->execute([$race_id]);
$rows = $query->fetchAll();
foreach ($rows as $row) 
{
...
{
    $query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_race_id = ? AND athletes.athlete_finishtime = ? AND athletes.athlete_sex = 'M' AND athletes.athlete_category = 'INF' ORDER BY athletes.athlete_started DESC");
