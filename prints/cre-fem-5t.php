<?php
  ...
  class PDF extends FPDF {
    ...
    function Header() {
      ...
      pdfHeader_H($this, $_GET['race_id'], 'F', 'Classificações Absolutos Femininos', 5);
    }
    // Page footer
    function Footer() {
  ...
  foreach ($athletes as $athlete) {
    $finishTime = gmdate('H:i:s', strtotime($athlete['athlete_t5'])-strtotime($athlete['athlete_t0']));
    ...
    $order->execute([$athlete['athlete_team_id'], $athlete['athlete_xtras'], 'F', $race_id]);
    ...
    $query->execute([$arriveOrder, $athlete['athlete_team_id'], $athlete['athlete_xtras'], 'F']);
  }
  ...
  foreach ($teams as $team) {
    if (!in_array($team['athlete_team_id'], $teamsArrived)) {
      ...
      foreach ($athletes as $athlete) {
        ...
        if($athlete['athlete_t1']=="-") $pdf->Cell(16,5,"-",1,0,'C',0);
        else $pdf->Cell(16,5,utf8_decode(gmdate('H:i:s',strtotime($athlete['athlete_t1']) - strtotime($athlete['athlete_t0']))),1,0,'C',0);
        if(($athlete['athlete_t2']=="-") || ($athlete['athlete_t1']=="-")) $pdf->Cell(16,5,"-",1,0,'C',0);
        else $pdf->Cell(16,5,utf8_decode(gmdate('H:i:s',strtotime($athlete['athlete_t2']) - strtotime($athlete['athlete_t1']))),1,0,'C',0);
        if(($athlete['athlete_t3']=="-")  || ($athlete['athlete_t2']=="-")) $pdf->Cell(16,5,"-",1,0,'C',0);
        else $pdf->Cell(16,5,utf8_decode(gmdate('H:i:s',strtotime($athlete['athlete_t3']) - strtotime($athlete['athlete_t2']))),1,0,'C',0);
        if(($athlete['athlete_t4']=="-")  || ($athlete['athlete_t3']=="-")) $pdf->Cell(16,5,"-",1,0,'C',0);
        else $pdf->Cell(16,5,utf8_decode(gmdate('H:i:s',strtotime($athlete['athlete_t4']) - strtotime($athlete['athlete_t3']))),1,0,'C',0);
        if(($athlete['athlete_t5']=="-") || ($athlete['athlete_t4']=="-")) $pdf->Cell(16,5,"-",1,0,'C',0);
        else $pdf->Cell(16,5,utf8_decode(gmdate('H:i:s',strtotime($athlete['athlete_t5']) - strtotime($athlete['athlete_t4']))),1,0,'C',0);
        $pdf->Cell(16,5,$athlete['athlete_finishtime'],1,0,'C',0);
        if ($posTeam === 3) {
          $pdf->Cell(16,5,$athlete['athlete_totaltime'],'R',0,'C',0);
        } elseif ($posTeam === 1) $pdf->Cell(16,5,'','T, R',0,'C',0);
        else $pdf->Cell(16,5,'','R',0,'C',0);
        if ($pos === 1) {
          if ($posTeam === 3) {
            $pdf->Cell(16,5,"-",'R',1,'C',0);
            $time_winner = $athlete['athlete_totaltime'];
          } else $pdf->Cell(16,5,'','R',1,'C',0);
        } else {
          if ($posTeam === 3) {
            $time = strtotime($athlete['athlete_totaltime']) - strtotime($time_winner);
            $pdf->Cell(16,5,gmdate('H:i:s', $time),'R',1,'C',0);
          } elseif ($posTeam === 1) $pdf->Cell(16,5,'','T, R',1,'C',0);
          else $pdf->Cell(16,5,'','R',1,'C',0);
        }
        $posTeam++;
      }
      ...
      for($i=0;$i<count($penalty);$i++) {
        ...
        foreach ($athletes as $athlete) {
          ...
          $pdf->Cell(16,5,$athlete['athlete_finishtime'],1,0,'C',0);
          if ($posTeam === 1) {
            $pdf->Cell(16,5,'','T, R',0,'C',0);
            $pdf->Cell(16,5,'','R',1,'C',0);
          } else {
            $pdf->Cell(16,5,'','L, R',0,'C',0);
            $pdf->Cell(16,5,'','R',1,'C',0);
          }
        }
      }
      $pdf->SetX(12);
      $pdf->Cell(272,5,'','T',0,'C',0);
      $pdf->Ln(1);
      $pos++;
    }
  }
  ...
  foreach ($teams as $team) {
    if (!in_array($team['athlete_team_id'], $teamsArrived)) {
      ...
      $query->execute([$team['athlete_team_id'], $team['athlete_xtras'], 'F']);
      $athletes = $query->fetchAll();
      foreach ($athletes as $athlete) {
        $pdf->SetX(12); 
        ...
        $pdf->Cell(16,5,$athlete['athlete_finishtime'],1,1,'C',0);
      }
      $pdf->Ln(1);
      $pos++;
    }
