<?php
  ...
  class PDF extends FPDF {
    ...
    function Header() {
      ...
      pdfHeader_V($this, $_GET['race_id'], 'M', 'Classificações Benjamins / Infantis Masculinos');
    }
    // Page footer
    function Footer() {
  ...
  for($i=0;$i<count($penalty);$i++) {
    $query = $db->prepare("SELECT athlete_team_id, athlete_bib FROM athletes WHERE athlete_finishtime=? AND athlete_race_id=? AND athlete_sex='M' AND athlete_bib LIKE '%C' ORDER BY athlete_started DESC");
