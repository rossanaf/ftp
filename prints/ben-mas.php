...
function Header()
{
    ...
    $querygun = $db->prepare("SELECT gunshot_benm FROM gunshots WHERE gunshot_race_id = ? LIMIT 1");
    ...
    $this->SetX(190);
    $this->Cell(10,5,$rowgun['gunshot_benm'],0,0,'R',true);
    
    $segment1 = ucwords($rowrace['youthrace_s1_ben'])." - ".$rowrace['youthrace_d1_ben'];
    if ($rowrace['youthrace_s2_ben'] !== 'n.a.')
    {
        $segment2 = ucwords($rowrace['youthrace_s2_ben'])." - ".$rowrace['youthrace_d2_ben'];
    }
    $segment3 = ucwords($rowrace['youthrace_s3_ben'])." - ".$rowrace['youthrace_d3_ben'];
    ...
    $this->Cell(190,8,utf8_decode("Classificações Benjamins Masculinos"),0,0,'C');
    $this->Ln(10);
    $this->SetDrawColor(0);
...
$query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_started >= 5 AND athletes.athlete_race_id = ? AND athletes.athlete_sex = 'M' AND athletes.athlete_category = 'BEN'");
...
foreach ($rows as $row) 
{
    ...
    // {
        $athlete_totaltime = gmdate('H:i:s', strtotime($row['athlete_finishtime'])-strtotime($rowrace['gunshot_benm']));
...
$query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_started >= 5 AND athletes.athlete_race_id = ? AND athletes.athlete_sex = 'M' AND athletes.athlete_category = 'BEN' ORDER BY athlete_finishtime ASC");
$query->execute([$race_id]);
$rows = $query->fetchAll();
foreach ($rows as $row) 
{
...
{
    $query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_race_id = ? AND athletes.athlete_finishtime = ? AND athletes.athlete_sex = 'M' AND athletes.athlete_category = 'BEN' ORDER BY athletes.athlete_started DESC");
