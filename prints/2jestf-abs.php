<?php
  ...
  foreach ($rowRaces as $race) {
    for($i=0;$i<2;$i++) {
      $gender = array('F', 'M');
      $queryPoints = $db->prepare("SELECT athlete_team_id, MIN(athlete_finishtime), athlete_bib FROM athletes WHERE athlete_started>=4 AND athlete_race_id=? AND athlete_sex=? GROUP BY athlete_team_id");
      $queryPoints->execute([$race['race_id'], $gender[$i]]);
      $rows = $queryPoints->fetchAll();
      foreach ($rows as $row) {
        $queryTeams = $db->prepare('INSERT INTO clubesj (clube, pontos) VALUES (?, ?)');
        $queryTeams->execute([$row['MIN(athlete_finishtime)'],$row['athlete_team_id']]);
      }
      $queryTeams2 = $db->prepare('SELECT pontos FROM clubesj ORDER BY clube ASC');
      $queryTeams2->execute();
      $rows = $queryTeams2->fetchAll();
      $pos = 1;
      foreach ($rows as $row) {
        if (($row['pontos'] != 1) && ($row['pontos'] != 2) && ($row['pontos'] != 3)) {
          if($pos === 1) $pontos = 210;
          ...
          $stmt_team->execute([$row['pontos'], $pontos]);
          $pos++;
        }
      }
      $stmtEmpty = $db->prepare('TRUNCATE clubesj');
      $stmtEmpty->execute();
      $stmtEmpty->closeCursor();
    }
  }
