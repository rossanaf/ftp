...
function Header()
{
    ...
    // Logo
    $this->Image('../images/ftp_logo.png',10,4,56);
    // Times bold 15
    ...
    $this->SetX(12);
    $this->Cell(24,4,utf8_decode("Local da Prova: "),0,0,'L',true);
    $this->SetX(138);
    ...
    $this->SetX(256);
    $this->Cell(10,5,'15:31:52',0,0,'R',true);
...
$query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_started >= '5' AND athletes.athlete_race_id = ? AND athletes.athlete_sex = 'X' ORDER BY athlete_finishtime ASC");
$query->execute([$race_id]);
$rows = $query->fetchAll();
foreach ($rows as $row) 
{
    ...
    if(($row['athlete_t2']=="-") || ($row['athlete_t1']=="-"))
       $pdf->Cell(20,4,"-",1,0,'C',$fill);
    else
       $pdf->Cell(20,4,utf8_decode(gmdate('H:i:s',strtotime($row['athlete_t2']) - strtotime($row['athlete_t1']))),1,0,'C',$fill);
    if(($row['athlete_t3']=="-")  || ($row['athlete_t2']=="-"))
       $pdf->Cell(20,4,"-",1,0,'C',$fill);
    else
       $pdf->Cell(20,4,utf8_decode(gmdate('H:i:s',strtotime($row['athlete_t3']) - strtotime($row['athlete_t2']))),1,0,'C',$fill);
    if(($row['athlete_t4']=="-")  || ($row['athlete_t3']=="-"))
       $pdf->Cell(20,4,"-",1,0,'C',$fill);
    else
       $pdf->Cell(20,4,utf8_decode(gmdate('H:i:s',strtotime($row['athlete_t4']) - strtotime($row['athlete_t3']))),1,0,'C',$fill);
    if(($row['athlete_t5']=="-") || ($row['athlete_t4']=="-"))
       $pdf->Cell(20,4,"-",1,0,'C',$fill);
    else
        $pdf->Cell(20,4,utf8_decode(gmdate('H:i:s',strtotime($row['athlete_t5']) - strtotime($row['athlete_t4']))),1,0,'C',$fill);
