...
function Header()
{
    ...
    $this->Cell(190,8,utf8_decode("Classificações Absolutos Geral Masculinos"),0,0,'C');
    $this->Ln(10);
    $this->SetDrawColor(0);
...
$query = $db->prepare("SELECT athlete_team_id, athlete_bib FROM athletes WHERE athlete_started >= 4 AND athletes.athlete_race_id = ? AND athletes.athlete_sex = 'M' ORDER BY athlete_finishtime ASC");
...
{
    $query = $db->prepare("SELECT athlete_team_id, athlete_bib FROM athletes WHERE athlete_finishtime = ? AND athletes.athlete_race_id = ? AND athletes.athlete_sex = 'M' AND athlete_bib LIKE '%C' ORDER BY athlete_finishtime ASC");
