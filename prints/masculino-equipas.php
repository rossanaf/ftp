<?php
  ...
  class PDF extends FPDF {
    function Header() {
      ...
      pdfHeader_V($this, $_GET['race_id'], 'M', 'Classificações Equipas Masculinas');
    }// Page footer
    function Footer() {
  ...
  $query = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athletes.athlete_started >= '5' AND athlete_race_id = ? AND athlete_sex = 'M'");
  ...
  $queryteams = $db->prepare("SELECT athlete_team_id, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id=teams.team_id WHERE athlete_started >= '5' AND athlete_race_id = ? AND athlete_sex = 'M' GROUP BY athlete_team_id HAVING COUNT(*) > 2");
  ...
  foreach ($teams as $row_clubes) {
    ...
    if (($row_clubes['athlete_team_id'] != 1) && ($row_clubes['athlete_team_id'] != 2) && ($row_clubes['athlete_team_id'] != 3) && ($estrangeira === false)) {
      $querytimes = $db->prepare("SELECT athletes.*, teams.team_name FROM athletes INNER JOIN teams ON athletes.athlete_team_id = teams.team_id WHERE athlete_team_id = ? AND athlete_started >= 5 AND athlete_race_id = ? AND athlete_sex = 'M' ORDER BY athlete_totaltime LIMIT 3");
