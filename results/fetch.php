<?php
	...
	$order_column = array("live_pos","live_firstname","live_lastname","live_bib","live_team","live_t1","live_t2","live_t3","live_t4","live_t5","live_finishtime");
	...
	if(isset($_POST["search"]["value"])){
		$query .= 'WHERE LOWER (live_team) LIKE "%'.$_POST["search"]["value"].'%" ';
		$query .= 'OR LOWER (live_firstname) LIKE "%'.$_POST["search"]["value"].'%" ';
		$query .= 'OR LOWER (live_lastname) LIKE "%'.$_POST["search"]["value"].'%" ';
		$query .= 'OR LOWER (live_team) LIKE "%'.$_POST["search"]["value"].'%" ';
		$query .= 'OR LOWER (live_finishtime) LIKE "%'.$_POST["search"]["value"].'%" ';
		$query .= 'OR live_bib LIKE "%'.$_POST["search"]["value"].'%" ';
	}
	...
	if(isset($_POST["order"]))
	{
	    ...
	    $query .= 'ORDER BY live_pos, live_started DESC, live_finishtime';
	}
	...
	{              
		...
		$flag="<img src='/images/flags/".$row['live_team'].".png' width='18px'>";
