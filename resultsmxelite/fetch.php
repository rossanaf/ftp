<?php
	...
	function get_total_all_records() {
		include ($_SERVER['DOCUMENT_ROOT']."/includes/db.php");
		$stmt = $db->query('SELECT * FROM live WHERE live_category="ELITE"');
		$result = $stmt->fetchAll();
		return $stmt->rowCount();
	}
  $output = array();
  $query = 'SELECT * FROM live LEFT JOIN teams ON live.live_team_id=teams.team_id WHERE live_category="ELITE" AND live_license=4 ';
   $query .= 'ORDER BY live_started DESC, live_pos, live_finishtime, live_t4, live_t3, live_t2, live_t1,LENGTH(live_bib), live_bib ';
    ...
    $i = 1;
    $stmtLegs = $db->prepare('SELECT live_finishtime FROM live WHERE live_bib=? AND live_category="ELITE"');
